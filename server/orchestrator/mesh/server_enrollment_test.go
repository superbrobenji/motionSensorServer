...
// enrollTestNode adds a pending node and returns the MAC string and public key.
func enrollTestNode(t *testing.T, ms *MeshServer) (macStr string, pubKey [32]byte) {
	...
	macStr = "aabbccddeeff"
	return macStr, pubKey
}

// decodeWrittenFrame reads the first protobuf frame from a MockSerialPort's write buffer.
...
func TestApproveEnrollment_UnknownMAC_ReturnsError(t *testing.T) {
	...
	err := ms.ApproveEnrollment("aabbccddeeff")
	if err == nil {
